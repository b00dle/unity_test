	...
	override public void Explode()
	{
		...
		if (_explodeDelay <= 0) //if the delayTime is zero
		{
			...
			foreach (Collider hit in _colliders) 
			{
				...
				if (hit.GetComponent<Rigidbody>() && hit.gameObject.layer != gameObject.layer)
				{
                    float force = power * size;
                    float rad = (radius * size);
                    float upwards = (4f * MyDetonator().upwardsBias * size);
                    ...
                    hit.GetComponent<Rigidbody>().AddExplosionForce(force, _explosionPosition, rad, upwards);

                    Damagable dmg = hit.gameObject.GetComponent<Damagable>();
                    if (dmg)
                        dmg.DealExplosionForceDamage(force, _explosionPosition, rad, upwards);
